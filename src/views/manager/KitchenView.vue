    ...
    <div class="flex items-center justify-between mb-6 border-b border-gray-700 pb-4">
      <div>
        ...
        <p class="text-gray-400 mt-1">
          {{
            viewMode === 'orders'
              ? `Ordini in preparazione: ${kitchenOrders.length}`
              : `Articoli totali da preparare`
          }}
        </p>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="bg-gray-800 p-1 rounded-lg flex">
          <button
            ...
            :class="
              viewMode === 'orders' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
            "
          >
            Per Ordine
          </button>
          <button
            ...
            class="px-4 py-2 rounded-md text-sm font-bold transition-colors"
            :class="
              viewMode === 'items' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
            "
          >
            Per Articolo
          </button>
    ...
    >
      ...
      >
        <div class="p-5 flex items-start gap-4">
          <div
            class="bg-blue-600 text-white font-bold text-3xl min-w-14 h-14 rounded-lg flex items-center justify-center shadow-lg"
          >
            {{ item.quantity }}
          </div>
          <div class="flex-1">
            ...
            <div v-if="item.notes.length > 0" class="mt-3 space-y-1">
              <p
                ...
                :key="idx"
                class="text-sm text-yellow-400 font-bold bg-yellow-900/30 p-1.5 rounded border border-yellow-900/50"
              >
                ⚠️ {{ note }}
              </p>
            </div>
            ...
            <div class="mt-4 pt-3 border-t border-gray-700">
              ...
              <div class="flex flex-wrap gap-1">
                <span
                  v-for="num in item.orderNumbers"
                  :key="num"
                  class="bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded text-xs font-mono"
                >
                  #{{ num }}
                </span>
              </div>
      ...
      >
        ...
        <div class="bg-gray-700 p-4 flex justify-between items-start">
          <div>
            ...
            <p class="text-gray-300 font-medium mt-1 truncate max-w-37.5">
        ...
        <!-- Items List -->
        <div class="p-4 flex-1 overflow-y-auto max-h-100">
          <ul class="space-y-3">
            <li
            ...
            >
              <div class="flex items-start gap-3">
                <span
                  class="bg-gray-600 text-white font-bold px-2 py-1 rounded text-lg min-w-8 text-center"
                >
                  {{ item.quantity }}
                </span>
                <div class="flex-1">
